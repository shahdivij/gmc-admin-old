...
const Residence = () => {
    ...
    const fetchData = async () => {
        ...
        const {data, error} = await GET(URL.CLUSTER.RESIDENCE)
        if(data)
            setData(data)
    ...
    const deleteData = async () => {
        const {data, error} = await DELETE(`${URL.CLUSTER.RESIDENCE}/${deleteId}`)
        if(data){
            setDeleteId(null)
            fetchData()
        }
    ...
    const handleFormSubmit = async () => {
        if(formData.name){
            if(showModal == 'add'){
                const {data, error} = await POST(URL.CLUSTER.RESIDENCE, formData)
                if(data){
                    setShowModal(false)
                    fetchData()
                }
            ...
            if(showModal == 'update'){
                const {data, error} = await PUT(URL.CLUSTER.RESIDENCE + "/" + selectedItemId, formData)
                if(data){
                    setShowModal(false)
                    fetchData()
                }
            ...
            if(!formData.name){
                setFormErrors(pre => ({...pre, name: "Please enter residence type name."}))
            }
        }
    ...
    return(
        <>
            <Container fluid className='pt-2'>
                <Row>
                    <Col lg={11}>
                        <h3 className='pageTitle'>Residence Type</h3>
                    </Col>
                    <Col lg={1}>
            ...
            <Modal size="md" centered show={showModal == 'add'} onHide={() => setShowModal(null)}>
                ...
                <Modal.Title>Add A New Residence Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                        ...
                        <Row>
                            <Col>
                                <Form.Group className="mb-3" controlId="formBasicText">
                                    <Form.Label  className={ formErrors["name"] && 'text-danger'}>Residence Type Name</Form.Label>
            ...
            <Modal size="md" centered show={showModal == 'update'} onHide={() => setShowModal(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Update Residence Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form autoComplete='off' aria-autocomplete='none'>
                        <Row>
            ...
            <Modal centered show={deleteId != null} onHide={() => setDeleteId(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Delete Residence Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>Are you sure you want to delete this Residence Type ?</p>
                </Modal.Body>
                <Modal.Footer>
