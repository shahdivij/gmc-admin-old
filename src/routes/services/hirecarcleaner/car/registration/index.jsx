...
const Registration = () => {
    ...
    const fetchData = async () => {
        const {data, error} = await GET(URL.CAR.REGISTRATION_TYPE)
        if(data)
            setData(data)
    }
    ...
    const deleteData = async () => {
        const {data, error} = await DELETE(`${URL.CAR.REGISTRATION_TYPE}/${deleteId}`)
        if(data){
            setDeleteId(null)
            fetchData()
        }
    ...
    const handleFormSubmit = async () => {
        if(formData.name){
            if(showModal == 'add'){
                const {data, error} = await POST(URL.CAR.REGISTRATION_TYPE, formData)
                if(data){
                    setShowModal(false)
                    fetchData()
                }
            ...
            if(showModal == 'update'){
                const {data, error} = await PUT(URL.CAR.REGISTRATION_TYPE + "/" + selectedItemId, formData)
                if(data){
                    setShowModal(false)
                    fetchData()
                }
            ...
            if(!formData.name){
                setFormErrors(pre => ({...pre, name: "Please enter registration type name."}))
            }
        }
    ...
    return(
        <>
            <Container fluid className='pt-2'>
                <Row>
                    <Col lg={11}>
                        <h3 className='pageTitle'>Car Registration Type</h3>
                    </Col>
                    <Col lg={1}>
            ...
            <Modal size="md" centered show={showModal == 'add'} onHide={() => setShowModal(null)}>
                ...
                <Modal.Title>Add A New Car Registration Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                        ...
                        <Row>
                            <Col>
                                <Form.Group className="mb-3" controlId="formBasicText">
                                    <Form.Label className={ formErrors["name"] && 'text-danger'}>Registration Type Name</Form.Label>
            ...
            <Modal size="md" centered show={showModal == 'update'} onHide={() => setShowModal(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Update Car Registration Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form autoComplete='off' aria-autocomplete='none'>
                        <Row>
            ...
            <Modal centered show={deleteId != null} onHide={() => setDeleteId(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Delete Registration Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
