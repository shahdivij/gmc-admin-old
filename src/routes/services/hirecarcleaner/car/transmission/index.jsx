...
const Transmission = () => {
    ...
    const fetchData = async () => {
        const {data, error} = await GET(URL.CAR.TRANSMISSION_TYPE)
        if(data)
            setData(data)
    ...
    const deleteData = async () => {
        const {data, error} = await DELETE(`${URL.CAR.TRANSMISSION_TYPE}/${deleteId}`)
        if(data){
            setDeleteId(null)
            fetchData()
        }
    ...
    const handleFormSubmit = async () => {
        if(formData.name){
            if(showModal == 'add'){
                const {data, error} = await POST(URL.CAR.TRANSMISSION_TYPE, formData)
                if(data){
                    setShowModal(false)
                    fetchData()
                }
            ...
            if(showModal == 'update'){
                const {data, error} = await PUT(URL.CAR.TRANSMISSION_TYPE + "/" + selectedItemId, formData)
                if(data){
                    setShowModal(false)
                    fetchData()
                }
            ...
            if(!formData.name){
                setFormErrors(pre => ({...pre, name: "Please enter transmission type name."}))
            }
        }
    ...
    return(
        <>
            <Container fluid className='pt-2'>
                <Row>
                    <Col lg={11}>
                        <h3 className='pageTitle'>Car Transmission Type</h3>
                    </Col>
                    <Col lg={1}>
            ...
            <Modal size="md" centered show={showModal == 'add'} onHide={() => setShowModal(null)}>
                ...
                <Modal.Title>Add A New Car Transmission Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                        ...
                        <Row>
                            <Col>
                                <Form.Group className="mb-3" controlId="formBasicText">
                                    <Form.Label  className={ formErrors["name"] && 'text-danger'}>Transmission Type Name</Form.Label>
            ...
            <Modal size="md" centered show={showModal == 'update'} onHide={() => setShowModal(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Update Car Transmission Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form autoComplete='off' aria-autocomplete='none'>
                        <Row>
            ...
            <Modal centered show={deleteId != null} onHide={() => setDeleteId(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Delete Transmission Type</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <p>Are you sure you want to delete this Transmission Type ?</p>
                </Modal.Body>
                <Modal.Footer>
