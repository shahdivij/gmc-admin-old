...
const Category = () => {
    ...
    const fetchData = async () => {
        const {data, error} = await GET(URL.CAR.CATEGORY)
        if(data)
            setData(data)
    }
    ...
    const deleteData = async () => {
        const {data, error} = await DELETE(`${URL.CAR.CATEGORY}/${deleteId}`)
        if(data){
            setDeleteId(null)
            fetchData()
        }
    ...
    const handleFormSubmit = async () => {
        if(formData.name){
            if(showModal == 'add'){
                const {data, error} = await POST(URL.CAR.CATEGORY, formData)
            ...
            if(showModal == 'update'){
                const {data, error} = await PUT(URL.CAR.CATEGORY + "/" + selectedItemId, formData)
            ...
            if(!formData.name){
                setFormErrors(pre => ({...pre, name: "Please enter category name."}))
            }
        }
    ...
    return(
        <>
            <Container fluid className='pt-2'>
                <Row>
                    <Col lg={11}>
                        <h3 className='pageTitle'>Car Category</h3>
                    </Col>
                    <Col lg={1}>
            ...
            <Modal size="md" centered show={showModal == 'add'} onHide={() => setShowModal(null)}>
                ...
                <Modal.Title>Add A New Car Category</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                        ...
                        <Row>
                            <Col>
                                <Form.Group className="mb-3" controlId="formBasicText">
                                    <Form.Label className={ formErrors["name"] && 'text-danger'}>Category Name</Form.Label>
            ...
            <Modal size="md" centered show={showModal == 'update'} onHide={() => setShowModal(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Update Car Category</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Form autoComplete='off' aria-autocomplete='none'>
                        <Row>
            ...
            <Modal centered show={deleteId != null} onHide={() => setDeleteId(null)}>
                <Modal.Header closeButton>
                <Modal.Title>Delete Car Category</Modal.Title>
                </Modal.Header>
                <Modal.Body>
